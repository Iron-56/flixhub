...
export default function WatchLater() {
	...
	const removeFromWatchlist = async () => {
		try {
			console.log("Removing movie from watchlist:", selectedMovie.id);
			const res = await fetch(`http://127.0.0.1:5000/watchlist?movieId=${selectedMovie.id}`, {
				method: 'DELETE',
				headers: {
					'Content-Type': 'application/json'
				},
			...
			const data = await res.json();
			if (res.status !== 200) {
				console.error('Error deleting movie from watch list', data);
				return;
			}
			setMovies(data);
		} catch (err) {
			console.error('Error on deleting movie from watch list', err);
		}
	...
	const emptyWatchlist = (
		<div className="flex flex-col items-center h-full justify-center p-8 text-center bg-[#151515] rounded-xl">
			<p className="text-xl text-gray-400 mt-4">Your watch later is empty!</p>
			<p className="text-md text-gray-500">Add some movies or shows to watch later.</p>
			<a className="mt-4 px-6 py-2 bg-[#FF0000] rounded-xl text-white hover:scale-105 duration-200" href="/">Add Movies</a>
		</div>
	);
	...
	return (
		<div className="flex flex-col gap-3 p-2 h-full sm:p-6 ">
			...
			{/* <PromptPopup isOpen={showPrompt} prompt={`Remove ${selectedMovie?.title} from watch later?`} accept="Remove" onClose={() => removeFromWatchlist()}/> */}
		</div>
